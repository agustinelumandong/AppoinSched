...
return new class extends Migration
{
    ...
    {
        // Create backup table to preserve existing data
        Schema::create('document_request_details_backup', function (Blueprint $table) {
            $table->id();
        ...
        DB::statement('INSERT INTO document_request_details_backup SELECT * FROM document_request_details');

        // Drop the original table
        Schema::dropIfExists('document_request_details');

        // Create the new normalized table with only common fields
        Schema::create('document_request_details', function (Blueprint $table) {
            $table->id();
            $table->foreignId('document_request_id')->constrained('document_requests')->onDelete('cascade');
            $table->enum('request_for', ['myself', 'someone_else'])->default('myself');
            $table->enum('certificate_type', ['birth', 'death', 'marriage'])->nullable();

            // Basic personal information (common to all certificate types)
            ...
            // Contact person information (for third-party requests)
            ...
            // Parent information (common to all certificate types)
    ...
    public function down(): void
    {
        ...
        // Restore from backup (this would recreate the original bloated structure)
        // Note: This is a complex rollback that would need to recreate all the original columns
        // For safety, we'll just drop the backup table
        Schema::dropIfExists('document_request_details_backup');
    }
};
